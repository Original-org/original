...
namespace original::details
{
    ...
    public:
         ...
         */
        constexpr StdIterator() = default;

        /**
         * @brief Construct from iterator.
         * @param it Iterator to adapt.
         */
        constexpr explicit StdIterator(iterator_type it) : it_(it) {}
        ...
        constexpr pointer operator->() const {
            if constexpr (requires { this->it_.operator->(); }) {
         ...
         */
        constexpr bool operator==(const StdIterator& rhs) const {
         ...
         */
        constexpr StdIterator& operator++()
        requires ForwardIterator<iterator_type>
        {
            ++this->it_;
            return *this;
         ...
         * @requires ForwardIterator<iterator_type>
         */
        constexpr StdIterator operator++(int)
        requires ForwardIterator<iterator_type>
        {
            auto tmp = *this;
            ++*this;
        ...
        constexpr StdIterator& operator+=(const T n)
        requires RandomAccessIterator<iterator_type> &&
         ...
         */
        template<SignedIntegralLike T>
        constexpr StdIterator operator+(const T n) const
        requires RandomAccessIterator<iterator_type> &&
        Convertible<NumberLikeType<T>, difference_type>
        {
            return StdIterator{ this->it_ + DifferenceType{n} };
        }
         ...
         */
        constexpr difference_type operator-(const StdIterator& rhs) const
        requires RandomAccessIterator<iterator_type>
        {
         ...
         * @requires RandomAccessIterator<iterator_type>
         */
        constexpr auto operator<=>(const StdIterator& rhs) const
        requires RandomAccessIterator<iterator_type>
        {
            return this->it_ <=> rhs.it_;
        }
         ...
         */
        template<SignedIntegralLike T>
        friend constexpr StdIterator operator+(T n, StdIterator it)
        requires RandomAccessIterator<iterator_type> &&
        Convertible<NumberLikeType<T>, difference_type>
        {
            return StdIterator{it.it_ + DifferenceType{n}};
        }
         ...
         */
        template<SignedIntegralLike T>
        friend constexpr StdIterator operator-(StdIterator it, T n)
        requires RandomAccessIterator<iterator_type> &&
        Convertible<NumberLikeType<T>, difference_type>
        {
            return StdIterator{it.it_ - DifferenceType{n}};
